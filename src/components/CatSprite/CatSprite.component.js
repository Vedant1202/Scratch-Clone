...
import { setSpriteEventListenerStatus } from '../../redux/events/events.actions';

/**
 * Component for Cat Sprite SVG.
 *
 * @component
 * @example
 * const moveX = 10
 ...
 * const angle = 10
 * const rotation = 10
 * const visibilitySay = false
 * const visibilityThink = false
 * return (
 *   <CatSprite moveX={moveX} moveY={moveY} angle={angle} rotation={rotation} visibilitySay={visibilitySay} visibilityThink={visibilityThink} />
 * )
 */
class CatSprite extends Component {
  /**
   * Contructor
   ...
   */
  handleClick = () => {
    const { setSpriteEventListenerStatus } = this.props;
    setSpriteEventListenerStatus(true);
    setTimeout(() => {
      setSpriteEventListenerStatus(false);
    }, 500);
  };
   ...
   */
  render() {
    let { moveX, moveY, rotation, angle, visibilitySay, visibilityThink } = this.props;
    return (
      <div
        onClick={this.handleClick}
        style={{
          position: 'relative',
          top: parseInt(moveY) + 'px',
          left: parseInt(moveX) + 'px',
          transform: `rotate(${parseInt(angle)}deg)`,
          cursor: 'pointer',
        }}
      >
        <div
          ...
          style={{
            display: visibilitySay ? 'block' : 'none',
          }}
        >
          Hello
        </div>
        <div
          ...
          style={{
            display: visibilityThink ? 'block' : 'none',
          }}
        >
          Hmmm...
        </div>
...
const mapDispatchToProps = (dispatch) => {
  return {
    setSpriteEventListenerStatus: (event) => {
      dispatch(setSpriteEventListenerStatus(event));
    },
  };
};

export default connect(null, mapDispatchToProps)(CatSprite);
